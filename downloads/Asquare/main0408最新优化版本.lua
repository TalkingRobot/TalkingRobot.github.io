-- ============================================================================
--  Asquare Ultra：本地 + 局域网联机 超级稳定优化版（含子弹破墙技能）
--  优化重点：对象池 + 墙壁网格 + 延迟删除 + 温和GC + 网络限频
--  原作者功能全部保留，可长时间稳定运行
-- ============================================================================
...
-- ============================================================================
POWERUP_TYPES = {
{id = "speed", name = "Speed Boost", desc = "Move faster", color = {0.3, 0.8, 1},
 apply = function(p) p.powerups.speed = (p.powerups.speed or 0) + 1 end},
{id = "damage", name = "Power Shot", desc = "+1 bullet damage", color = {1, 0.3, 0.3},
 apply = function(p) p.powerups.damage = (p.powerups.damage or 0) + 1 end},
{id = "build", name = "Builder", desc = "+3 blocks per kill", color = {0.3, 1, 0.3},
 apply = function(p) p.powerups.build = (p.powerups.build or 0) + 1 end},
{id = "multishot", name = "Multi Shot", desc = "Shoot 3 bullets", color = {1, 0.8, 0.2},
 apply = function(p) p.powerups.multishot = (p.powerups.multishot or 0) + 1 end},
{id = "pierce", name = "Piercing", desc = "Bullets pierce zombies", color = {0.8, 0.3, 1},
 apply = function(p) p.powerups.pierce = (p.powerups.pierce or 0) + 1 end},
{id = "wallbreak", name = "Wall Breaker", desc = "Bullets destroy walls", color = {0.7, 0.7, 0.7},
 apply = function(p) p.powerups.wallbreak = (p.powerups.wallbreak or 0) + 1 end},
{id = "regen", name = "Regeneration", desc = "Slow HP regen", color = {1, 0.5, 0.8},
 apply = function(p) p.powerups.regen = (p.powerups.regen or 0) + 1 end}
}
powerupSelection = {active = false, options = {}, selectedIndex = 1, milestone = 0}

-- ============================================================================
--  网络模块
-- ============================================================================
net = {
  role = nil, roomName = "", udpBcast = nil, udpGame = nil, tcp = nil,
  peer = nil, lastBeat = 0, remoteInput = {}, roomConflict = false, sendTimer = 0
}
...
function wallAt(x, y) return wallGrid[y] and wallGrid[y][x] or nil end
function towerWallAt(x, y) return towerWallGrid[y] and towerWallGrid[y][x] or nil end

local function addWall(x, y, hp)
  local w = {x = x, y = y, hp = hp or 5}
  table.insert(walls, w)
  if not wallGrid[y] then wallGrid[y] = {} end
  wallGrid[y][x] = w
end

local function removeWall(w)
  for i = #walls, 1, -1 do
    if walls[i] == w then table.remove(walls, i) break end
  end
  if wallGrid[w.y] then wallGrid[w.y][w.x] = nil end
end

local function addTowerWall(x, y, hp)
  local tw = {x = x, y = y, hp = hp or 5}
  table.insert(towerWalls, tw)
  if not towerWallGrid[y] then towerWallGrid[y] = {} end
  towerWallGrid[y][x] = tw
end

local function removeTowerWall(tw)
  for j = #towerWalls, 1, -1 do
    if towerWalls[j] == tw then table.remove(towerWalls, j) break end
  end
  if towerWallGrid[tw.y] then towerWallGrid[tw.y][tw.x] = nil end
end
...
local function getBullet()
  if #bulletPool > 0 then
    ...
    b.x, b.y, b.dx, b.dy, b.damage, b.pierce, b.breaksWalls = 0, 0, 0, 0, 1, false, false
    return b
  end
  return {x=0, y=0, dx=0, dy=0, damage=1, pierce=false, breaksWalls=false}
end
...
local function getZombie()
  if #zombiePool > 0 then return table.remove(zombiePool) end
  return {x=0, y=0, hp=0, score=0, color={1,0,0}, blocks=1, moveTimer=0}
end

local function releaseZombie(z)
  table.insert(zombiePool, z)
end

local function clearEntities()
  for _, b in ipairs(bullets) do releaseBullet(b) end
  bullets = {}
  for _, z in ipairs(zombies) do releaseZombie(z) end
  zombies = {}
  walls = {}
  towerWalls = {}
  for y = 0, MAP_H - 1 do
...
function generateRandomMap()
  ...
  for _, off in ipairs(crossOffsets) do
    ...
    if inMap(tx, ty) then addTowerWall(tx, ty, 5) end
  end
end
...
function checkPowerupMilestone()
  local milestone = math.floor(score / 100) * 100
...
function spawnZombie()
  ...
  local z = getZombie()
  z.x, z.y, z.hp, z.score, z.color, z.blocks, z.moveTimer = x, y, t.hp, t.score, t.color, t.blocks, ZOMBIE_MOVE_INTERVAL
...
function netInit()
  ...
  net.lastBeat, net.remoteInput, net.sendTimer = 0, {}, 0
end
...
function love.load()
  ...
  for i = 1, MAX_POOL_SIZE do
    table.insert(bulletPool, {x=0,y=0,dx=0,dy=0,damage=1,pierce=false,breaksWalls=false})
    table.insert(zombiePool, {x=0,y=0,hp=0,score=0,color={1,0,0},blocks=1,moveTimer=0})
  end
  for y = 0, MAP_H - 1 do
    wallGrid[y] = {}
    towerWallGrid[y] = {}
...
function love.keypressed(key, scancode, isrepeat)
  ...
  if gameState == "start" then
    if key == "tab" then
      ...
      if currentMenuSelection < 1 then currentMenuSelection = 3 end
      if currentMenuSelection > 3 then currentMenuSelection = 1 end
      SFX.menu_select:play()
  ...
  if gameState == "play_submenu" then
    if key == "tab" then
      ...
      if currentPlaySelection < 1 then currentPlaySelection = 3 end
      if currentPlaySelection > 3 then currentPlaySelection = 1 end
      SFX.menu_select:play()
    elseif key == "return" or key == "space" then
  ...
  if gameState == "settings" then
    ...
    if key == "tab" then
      ...
      if currentSettingSelection < 1 then currentSettingSelection = 13 end
      if currentSettingSelection > 13 then currentSettingSelection = 1 end
      SFX.menu_select:play()
    elseif key == "return" or key == "space" then
  ...
  if gameState == "playing" then
    local function handlePlayerInput(pl)
      if pl.hp <= 0 then return end
      local c = pl.controls
      ...
      if key == c.shoot then 
        ...
        if multishot > 0 then
          local b1 = getBullet(); b1.x, b1.y, b1.dx, b1.dy, b1.damage, b1.pierce, b1.breaksWalls = pl.x, pl.y, pl.dirx, pl.diry, dmg, prc, breaksWalls; table.insert(bullets, b1)
          local side1_dx, side1_dy = -pl.diry, pl.dirx 
          local side2_dx, side2_dy = pl.diry, -pl.dirx
          local b2 = getBullet(); b2.x, b2.y, b2.dx, b2.dy, b2.damage, b2.pierce, b2.breaksWalls = pl.x, pl.y, side1_dx, side1_dy, dmg, prc, breaksWalls; table.insert(bullets, b2)
          local b3 = getBullet(); b3.x, b3.y, b3.dx, b3.dy, b3.damage, b3.pierce, b3.breaksWalls = pl.x, pl.y, side2_dx, side2_dy, dmg, prc, breaksWalls; table.insert(bullets, b3)
        else
          local b = getBullet(); b.x, b.y, b.dx, b.dy, b.damage, b.pierce, b.breaksWalls = pl.x, pl.y, pl.dirx, pl.diry, dmg, prc, breaksWalls; table.insert(bullets, b)
        end
...
function startLocalGame()
  ...
  if musicEnabled then bgm:play() end
  if gameMode == "1P" then players[2].hp = 0 end
end
...
function love.update(dt)
  ...
  if gameMode == "online" then
    ...
    net.sendTimer = (net.sendTimer or 0) + dt
    if net.sendTimer > 0.033 then  -- ≈30Hz
      netSendInput()
      net.sendTimer = 0
    end
  end
  ...
  for i = #bullets, 1, -1 do
    ...
    if not inMap(math.floor(b.x), math.floor(b.y)) then
      releaseBullet(b)
  ...
  for zi = #zombies, 1, -1 do
    local z = zombies[zi]
    z.moveTimer = z.moveTimer - dt
    if z.moveTimer <= 0 then
      z.moveTimer = ZOMBIE_MOVE_INTERVAL
      local d1 = (players[1].hp > 0) and (math.abs(players[1].x - z.x) + math.abs(players[1].y - z.y)) or 999
      local d2 = 999
      if gameMode == "2P" or (gameMode == "online" and players[2].hp > 0) then
        d2 = (players[2].hp > 0) and (math.abs(players[2].x - z.x) + math.abs(players[2].y - z.y)) or 999
      end
      local target = (d1 < d2) and players[1] or players[2]
      local dx = target.x > z.x and 1 or (target.x < z.x and -1 or 0)
      local dy = target.y > z.y and 1 or (target.y < z.y and -1 or 0)
      local nx, ny = z.x + dx, z.y + dy
      local hit = false
      for i, p in ipairs(players) do
        if gameMode == "1P" and i == 2 then break end
        if p.hp > 0 and nx == p.x and ny == p.y then
      ...
      if not hit then
        ...
        if w then 
          w.hp = w.hp - 1
          if w.hp <= 0 then removeWall(w) end
        elseif tw then
          tw.hp = tw.hp - 1
          if tw.hp <= 0 then removeTowerWall(tw) end
    ...
    if not inMap(bx, by) then
      ...
      if b.breaksWalls then
        ...
        if w then
          ...
          if not b.pierce then removed = true end
        end
        if not removed then
  ...
  if gameMode == "online" and net.peer then
    ...
    if p2.hp > 0 then
      ...
      if remote.shoot then 
        ...
        if multishot > 0 then
          local b1 = getBullet(); b1.x,b1.y,b1.dx,b1.dy,b1.damage,b1.pierce,b1.breaksWalls = p2.x,p2.y,p2.dirx,p2.diry,dmg,prc,breaksWalls; table.insert(bullets,b1)
          local side1_dx, side1_dy = -p2.diry, p2.dirx
          local side2_dx, side2_dy = p2.diry, -p2.dirx
          local b2 = getBullet(); b2.x,b2.y,b2.dx,b2.dy,b2.damage,b2.pierce,b2.breaksWalls = p2.x,p2.y,side1_dx,side1_dy,dmg,prc,breaksWalls; table.insert(bullets,b2)
          local b3 = getBullet(); b3.x,b3.y,b3.dx,b3.dy,b3.damage,b3.pierce,b3.breaksWalls = p2.x,p2.y,side2_dx,side2_dy,dmg,prc,breaksWalls; table.insert(bullets,b3)
        else
          local b = getBullet(); b.x,b.y,b.dx,b.dy,b.damage,b.pierce,b.breaksWalls = p2.x,p2.y,p2.dirx,p2.diry,dmg,prc,breaksWalls; table.insert(bullets,b)
        end
        SFX.shoot:stop(); SFX.shoot:play() 
      end
